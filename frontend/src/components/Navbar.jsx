...
const Navbar = () => {
    ...
    const handleLogout = async () => {
        try {
            await axios.post("http://localhost:3000/api/auth/logout"); // optional
        } catch (err) {
    ...
    // Close menu if clicking outside drawer
        ...
        <>
            <header className="fixed top-0 left-0 w-full z-50 bg-black/40 backdrop-blur-md border-b border-[#c2995a]/30 shadow-[0_4px_15px_rgba(194,153,90,0.15)]">
                <div className="max-w-7xl mx-auto flex justify-between items-center px-5 md:px-6 py-3 md:py-4">
                    ...
                    <NavLink
                        to="/home"
                        className="text-[#c2995a] text-lg md:text-2xl font-semibold tracking-wide hover:text-[#d6aa61] transition-colors"
                    >
                        DivineVerse
                    </NavLink>
                        ...
                        {navItems.map((item) =>
                            item.action ? (
                                ...
                                <NavLink
                                    ...
                                    to={item.link}
                                    className={({ isActive }) =>
                                        `flex items-center gap-2 text-base transition-colors duration-200 ${isActive
                                            ? "text-[#c2995a]"
                                ...
                                {navItems.map((item) =>
                                    item.action ? (
                                        ...
                                        <NavLink
                                            ...
                                            onClick={() => setMenuOpen(false)}
                                            className={({ isActive }) =>
                                                `flex items-center gap-3 px-3 py-3 rounded-lg text-base transition-all duration-200 ${isActive
